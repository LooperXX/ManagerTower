...
def albef_transform_randaug_pretrain(size):
    return Compose([                        
            RandomResizedCrop(size, scale=(0.2, 1.0),interpolation=transforms.InterpolationMode.BICUBIC),
            RandomHorizontalFlip(),
            RandomAugment(2, 7, isPIL=True, augs=['Identity','AutoContrast','Brightness','Sharpness','Equalize',
...
def albef_transform_randaug(size):
    return Compose([                        
...
def albef_transform_randaug_wc(size): # wc: with color changes
    return Compose([
            RandomResizedCrop(size, scale=(0.5, 1.0),interpolation=transforms.InterpolationMode.BICUBIC),
            RandomHorizontalFlip(),
            RandomAugment(2, 7, isPIL=True, augs=['Identity','AutoContrast','Brightness','Sharpness','Equalize',
                                              'ShearX', 'ShearY', 'TranslateX', 'TranslateY', 'Rotate', 'Solarize', 'Color', 'Contrast', 'Posterize']),
            lambda image: Image.fromarray(image).convert("RGB"),
            ToTensor(),
            Normalize((0.48145466, 0.4578275, 0.40821073), (0.26862954, 0.26130258, 0.27577711)),
        ])

def albef_transform_randaug_wohf(size): # remove horizontal flip for vqa
    return Compose([                        
            RandomResizedCrop(size, scale=(0.5, 1.0),interpolation=transforms.InterpolationMode.BICUBIC),
            # RandomHorizontalFlip(),
            RandomAugment(2, 7, isPIL=True, augs=['Identity','AutoContrast','Brightness','Sharpness','Equalize',
