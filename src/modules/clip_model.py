...
class Transformer(nn.Module):
    def __init__(self, width: int, layers: int, heads: int, attn_mask: torch.Tensor = None, model_type: str = "MT", vit_remove_last: bool = False):
        super().__init__()
        self.width = width
        self.layers = layers
        if vit_remove_last:
...
class VisualTransformer(nn.Module):
    def __init__(self, input_resolution: int, patch_size: int, width: int, layers: int, heads: int, output_dim: int, resolution_after: int, model_type: str = "MT", vit_layernorm_shared: bool = True, vit_remove_last: bool = False):
        super().__init__()
...
def build_model(name, resolution_after=224, model_type="MT", vit_layernorm_shared=True, vit_remove_last=False):
    if name in _MODELS:
