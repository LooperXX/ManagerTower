    ...
    def get_uni_modal_features(self, batch, fusion_features=False, itc=False):
        ...
        if itc:
            if self.hparams.config["meter_fusion"]:
                ...
                unimodal_feats_text = F.normalize(self.cross_modal_itc_text_head(text_embeds[:, 0, :]), dim=-1, p=2)
                unimodal_feats_image = F.normalize(self.cross_modal_itc_image_head(image_embeds[:, 0, :]), dim=-1, p=2)
