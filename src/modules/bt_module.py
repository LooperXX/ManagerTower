...
class BTTransformer(pl.LightningModule):
    ...
    def infer(
        ...
        if irtr_len_image > 0:
            ...
            y = y.unsqueeze(1).expand(_bs, irtr_len_image, _L, _D).contiguous().view(-1, _L, _D)
        ...
        for i in range(split_index, len(self.text_transformer.encoder.layer)):
            ...
            if irtr_len_image > 0:
                y1_ = image_link_tower(image_embeds_.unsqueeze(1).expand(_bs, irtr_len_image, _L, _D).contiguous().view(-1, _L, _D), y1)
            else:
